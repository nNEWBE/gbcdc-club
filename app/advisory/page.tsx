...
import { Linkedin, Mail, GraduationCap, Building2, Globe, BookOpen, Award, Quote } from "lucide-react";
...
const advisors = [
  {
    ...
    initials: "AR",
    expertise: ["University Leadership", "Academic Policy", "Institutional Development"],
    message:
  ...
  },
  {
    ...
    initials: "FR",
    expertise: ["Communication Skills", "Academic Writing", "Professional Development"],
    message:
          ...
          <div className="group relative bg-neutral-50 rounded-3xl border border-neutral-200 p-8 md:p-12 overflow-hidden hover:shadow-xl hover:shadow-black/5 transition-all duration-500">
            ...
            <div className="relative z-10 grid md:grid-cols-3 gap-8 items-center">
              <div className="flex flex-col items-center text-center">
                <div className={`w-32 h-32 rounded-full bg-gradient-to-br ${advisors[0].gradient} flex items-center justify-center text-white text-4xl font-bold shadow-xl shadow-primary/20 mb-4`}>
                  {advisors[0].initials}
                </div>
                <h3 className="text-xl font-bold text-black">{advisors[0].name}</h3>
              ...
              <div className="md:col-span-2">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-xs font-semibold text-neutral-400 uppercase tracking-wider">Chairman&apos;s Message</span>
                </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {advisors.slice(1).map((advisor, i) => (
            ...
            >
              ...
              <div className="flex flex-col items-center text-center">
                <div className={`w-20 h-20 rounded-full bg-gradient-to-br ${advisor.gradient} flex items-center justify-center text-white text-xl font-bold mb-4 group-hover:scale-110 transition-transform duration-500 shadow-lg`}>
                  {advisor.initials}
                </div>

                <h3 className="text-base font-bold text-black">{advisor.name}</h3>
                <p className="text-xs font-semibold text-primary mt-1">{advisor.title}</p>
                <p className="text-xs text-neutral-400 mt-0.5">{advisor.designation}</p>
                ...
                <div className="mt-4 flex flex-wrap justify-center gap-1.5">
                  {advisor.expertise.map((exp) => (
                    <span key={exp} className="px-2 py-1 bg-neutral-50 rounded-full text-xs text-neutral-500 font-medium">
                      {exp}
                    </span>
                  ))}
                </div>
                ...
                <div className="mt-5 flex items-center gap-2">
                  <a href="#" className="w-9 h-9 rounded-full bg-neutral-100 hover:bg-primary hover:text-white flex items-center justify-center transition-all duration-300 cursor-pointer" aria-label={`${advisor.name}'s LinkedIn`}>
                    <Linkedin size={14} />
                  </a>
                  <a href="#" className="w-9 h-9 rounded-full bg-neutral-100 hover:bg-primary hover:text-white flex items-center justify-center transition-all duration-300 cursor-pointer" aria-label={`Email ${advisor.name}`}>
                    <Mail size={14} />
                  </a>
                </div>
              </div>
